...
class ModelPusher:
    ...
    def push_model(self, model, r2_score: float, mae_score: float) -> bool:
        """
        Push model to mlflow registry and promote to production if it perform better.
        """

        try:
            logging.info("Checking the existing model performance...")
            ...
            with mlflow.start_run():
                ...
                if not latest_versions:
                    raise Exception("no newly regsitered model found...")
            ...
            logging.error(f"error occured while pushing and promoting the model {e}")
